...
# package:// mesh paths that need to be rewritten for Isaac's importer.  The
# repository may omit large STL files, but the real Isaac runtime checkout is
# expected to contain per-link meshes such as base_link.STL/link1_L.STL.  The
# runtime Isaac-ready copy therefore preserves each URDF mesh filename and never
# substitutes another mesh unless REDIRECT_MISSING_MESHES_TO_FALLBACK is enabled.
DEFAULT_LOWER_LIMIT = -math.pi
DEFAULT_UPPER_LIMIT = math.pi
DEFAULT_EFFORT = 250.0
DEFAULT_VELOCITY = 2.0
REDIRECT_MISSING_MESHES_TO_FALLBACK = False
...
def ensure_isaac_ready_urdf() -> Path:
    ...
    for mesh in root.findall(".//mesh"):
        ...
        expected_mesh_path = (MESH_DIR / mesh_name).resolve()
        mesh_exists = mesh_name in available_mesh_paths

        if mesh_exists or filename.startswith("package://") or not Path(filename).is_absolute():
            # Preserve the URDF's explicit mesh filename, but make package/relative
            # paths absolute so Isaac's importer does not need a ROS package map.
            mesh.attrib["filename"] = str(expected_mesh_path)
            mesh_rewrites += 1

        if not mesh_exists:
            missing_meshes.append(mesh_name)
            if REDIRECT_MISSING_MESHES_TO_FALLBACK and fallback_mesh_path is not None:
    ...
    unique_missing_meshes = sorted(set(missing_meshes))
    print(
        f"[URDF] wrote Isaac-ready copy: {ISAAC_READY_URDF_PATH}\n"
        f"       widened_limits={widened_limits}, rewritten_mesh_paths={mesh_rewrites}, "
        f"missing_mesh_names={len(unique_missing_meshes)}, redirected_missing_meshes={mesh_redirects}"
    )
    if unique_missing_meshes:
        print(
            "[URDF][WARN] These mesh files are referenced by the URDF but are not "
            f"present in this checkout's mesh folder ({MESH_DIR}): {unique_missing_meshes}. "
            "The filenames were preserved in the generated URDF; copy the real STL "
            "files into the mesh folder before running Isaac import."
        )
